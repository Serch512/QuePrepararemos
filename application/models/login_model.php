    ...
    public function registerNewUser()
    {
       ...
       if (empty($_POST['user_name'])) {
        ...
        //} //elseif (empty($_POST['group_id'])) {
            ...
            $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_DOES_NOT_FIT_PATTERN;
        } elseif (strlen($_POST['user_lastName']) > 64 OR strlen($_POST['user_lastName']) < 2) {
            $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_TOO_SHORT_OR_TOO_LONG;
        } elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_lastName'])) {
            $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_DOES_NOT_FIT_PATTERN;
        //} //poner el de group_id
        //elseif (strlen($_POST['group_id']) > 64 OR strlen($_POST['group_id']) < 2) {
          //  $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_TOO_SHORT_OR_TOO_LONG;
       // } elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['group_id'])) {
         //   $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_DOES_NOT_FIT_PATTERN;
        }  
         elseif (empty($_POST['user_email'])) {
            ...
            AND strlen($_POST['user_lastName']) <= 64
            AND strlen($_POST['user_lastName']) >= 2
            AND preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_lastName'])
            //AND !empty($_POST['group_id'])
            //AND strlen($_POST['group_id']) <= 64
            //AND strlen($_POST['group_id']) >= 2
            //AND preg_match('/^[0-9]{2,64}$/i', $_POST['group_id'])
...
//exit;
           ...
           // $group_id = strip_tags($_POST['group_id']);
            ...
            $sql = "INSERT INTO users (user_name, user_lastName, user_password_hash, user_email, user_creation_timestamp, user_activation_hash, user_provider_type)
                    VALUES (:user_name,:user_lastName , :user_password_hash, :user_email, :user_creation_timestamp, :user_activation_hash, :user_provider_type)";
